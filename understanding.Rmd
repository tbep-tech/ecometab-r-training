...
The next step is to "map" variables in your dataset to the visual properties or "aesthetics" in the plot (i.e., x variable, y variable, etc.).   The `mapping` argument is defined with `aes()`, and the `x` and `y` arguments of `aes()` specify which variables to map to the x and y axes. The function looks for the mapped variable in the `data` argument, in this case, `apaeco`.  
...
We can add a date column using the `date` function in lubridate, group by day using the `group_by` function from dplyr, then take average water temperature using the `summarise` function in dplyr. 
...
1. Combine the nutrients and metabolism data by the floored dates using `inner_join` and `by = 'dateflr'`. 
