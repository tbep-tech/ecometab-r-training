...
1. Simultaneously rename and select the date/time columns (`DateTimeStamp`), water temperature (`Temp`), salinity (`Sal`), dissolved oxygen (`DO_obs`), air temperature (`ATemp`), barometric pressure (`BP`), wind speed (`WSpd`), and tidal height (`Tide`) columns in the Sapelo Island dataset with the `select` function from dplyr.  Don't forget to assign the new dataset to an object in your workspace (with `<-`).
