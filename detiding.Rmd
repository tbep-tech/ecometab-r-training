...
What we get is a time series of the correlation of tidal height with the sun angle.  We can expect the detiding method to work best when this correlation is zero and it will not as effective as the correlation changes from zero.  So, in this case, detiding may not work as well in middle to late summer and early spring.  For periods of the year when detiding may not work, interpret the results with caution, or simply do not use detiding during those parts of the year. 

Now that we have an expectation for when detiding would work well, we can use the `wtreg` function to detide the DO time series. The same inputs for `evalcor` are used for `wtreg`.  The rest of the arguments for `wtreg` are left at their default value, including the window widths for weighted regression.  Running the function may also take a minute or two. 
...
1. Run `evalcor` using `tz = 'America/Jamaica'`, `lat = ''`, and `long = ''`.
1. From the plot, what times of year might you expect detiding to not work very well? 
